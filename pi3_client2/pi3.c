...
static long pi3_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	switch(cmd)
	{
		case PI3_DOOROPEN:
...
static int pi3_open(struct inode *inode, struct file *file)
{
	printk("pi3 open\n");
	return 0;
}

static int pi3_release(struct inode *inode, struct file *file)
{
	printk("pi3 release\n");
	return 0;
}
...
struct file_operations pi3_char_fops = 
{
	.unlocked_ioctl = pi3_ioctl,
	.open = pi3_open,
	.release = pi3_release,
};
...
static int __init pi3_init(void){
	
	//ioctl 초기화 
	alloc_chrdev_region(&dev_num,0,1,DEV_NAME);
	cd_cdev = cdev_alloc();
	cdev_init(cd_cdev, &pi3_char_fops);
...
static void __exit pi3_exit(void){
	gpio_free(PIN1);
	gpio_free(PIN2);
	gpio_free(PIN3);
	gpio_free(PIN4);
...
module_init(pi3_init);
module_exit(pi3_exit);
